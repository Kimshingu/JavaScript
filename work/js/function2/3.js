...
var obj2 = new Object(); // 비 권장방법
console.log(obj2);

// 필요하다면 언제든지 객체의 프로퍼티를 조작할 수 있다.
obj.b = 20;
console.log(obj);
...
function Vehicle(){
  ...
  // 자바의 this는 고정이지만 JS this는 함수를 호출하는 맥락에 따라 번경되므로 주의가 필요하다.
  this.color ="red";
...
// __proto__: 이 히든 프로퍼티는 부모를 가리키는 용도의 프로퍼티로써 객체 생성 시 자동으로 추가된다
// 새 객체는 생성자 함수 Vehicle을 사용하여 만들어졌으므로
// 새 객체의 부모는 Vehicle 함수가 가진 prototype 객체가 된다.


// prototype : 이 프로퍼티가 가리키는 객체는 자식객체에 제공하고자 하는 프로퍼티를
// 부모입장에서 제공하기 위해 존재하는 객체로써
// 모든 일반 함수는 자동으로 이 속성을 가진다.
